...
if ($aksi == "daftar")
	...
	//tampilkan daftar...
	$qku = mysql_query("SELECT * FROM m_pendidikan ".
							"ORDER BY nama ASC");
...
if ($aksi == "simpan2")
	...
	if (empty($e_nama))
		...
		{
		//update
		mysql_query("UPDATE m_pendidikan SET nama = '$e_nama' ".
						"WHERE kd = '$e_kd'");
...
if ($aksi == "simpan")
	...
	if (empty($e_nama))
		...
		{
		//cek dulu....
		$qcc = mysql_query("SELECT * FROM m_pendidikan ".
								"WHERE nama = '$e_nama'");
		...
		if (empty($tcc))
			{
			//insert
			mysql_query("INSERT INTO m_pendidikan(kd, nama, postdate) VALUES ".
							"('$x', '$e_nama', '$today');");
...
if ($aksi == "hapus")
	...
	//hapus
	mysql_query("DELETE FROM m_pendidikan ".
					"WHERE kd = '$kd'");
