...
if ($aksi == "simpan2")
	...
	if (empty($e_nama))
		...
		{
		//update
		mysqli_query($koneksi, "UPDATE m_poli SET nama = '$e_nama' ".
						"WHERE kd = '$e_kd'");
...
if ($aksi == "simpan")
	...
	if (empty($e_nama))
		...
		{
		//cek dulu....
		$qcc = mysqli_query($koneksi, "SELECT * FROM m_poli ".
								"WHERE nama = '$e_nama'");
		...
		if (empty($tcc))
			{
			//insert
			mysqli_query($koneksi, "INSERT INTO m_poli(kd, nama, postdate) VALUES ".
							"('$x', '$e_nama', '$today');");
...
if ($aksi == "hapus")
	...
	//hapus
	mysqli_query($koneksi, "DELETE FROM m_poli ".
					"WHERE kd = '$kd'");
